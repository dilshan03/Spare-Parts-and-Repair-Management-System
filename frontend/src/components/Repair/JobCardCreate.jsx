...
const JobCardCreate = () => {
  ...
  useEffect(() => {
    const fetchMechanics = async () => {
      ...
      try {
        ...
        const mechanicsList = (response.data || []).filter(employee => 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosInstance.post("/jobCards", {
        repairId,
        assignedMechanic,
        jobs,
      });
      ...
      navigate("/RepairadminDash/AllRepair"); // Navigate back to the main page
    } catch (error) {
